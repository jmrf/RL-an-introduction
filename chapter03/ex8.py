...
def init_world():
    ...
    for s in range(world_size**2):
        ...
        for a in range(len(actions)):
            ...
            reward = 0
            if s_prime < 0 or s_prime > world_size**2 - 1:
                reward = -1     # going out of the grid
                s_prime = s     # stay in the same grid cell
            elif s_prime == 1:  # special case from state A -> A'
                reward = 10     # state A: (0, 1)
                s_prime = GridWorld.coordinates_2_state([4, 1], world_size)      # state A': (4, 1)
            elif s_prime == 3:  # special case from state B -> B'
                reward = 5      # state B: (0, 3)
                s_prime = GridWorld.coordinates_2_state([2, 3], world_size)      # state A': (2, 3)
...
def compute_value_function(world, policy, gamma=0.9):
    pass
